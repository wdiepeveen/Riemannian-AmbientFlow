...
class DeformedGaussianSolver(EncoderDecoderSolver):
    def __init__(self, forward_operator, diffeomorphism, sparsity_level, lambd):
        super().__init__(forward_operator, diffeomorphism, sparsity_level)
        self.psi = LearnableDiagonalQuadratic(self.d, use_softplus=True)
        self.lambd = lambd
    
    def reconstruction_loss(self,y):
        z = self.encode(y)
        z[:,-1] = 0. # TODO remove this/implement sparsity leven and do things there
